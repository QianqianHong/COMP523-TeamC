...
test('add record', async () => {
  ...
  const personLeveltest = new Object({
    uid: 'name 1',
    phase_of_service: 'PHASE OF SERVICE STRING',
    first_date: 'FIRST DATE STRING',
    ipe_with_vr: true,
    vr_id: 1,
    county_id: 1,
    current_clinical_services: ['SERVICE 1', 'SERVICE 2'],
    clinical_services_agency: 'CLINICAL SERVICES AGENCY STRING',
  })
...
test('add unformatted record', async () => {
  ...
  const personLeveltest = new Object({
    ...
    vr_id: 2,
    county_id: 2,
    current_clinical_services: [
      'SERVICE 1',
      'SERVICE 2',
      'MISSING CLINICAL SERVICES AGENCY',
    ],
  })
  await personLevelServices.addRecordToDB(personLeveltest).then((status) => {
    expect(status).not.toBeNull()
...
test('update record', async () => {
  ...
  const personLeveltest = new Object({
    ...
    current_clinical_services: [
      'SERVICE 1',
      'SERVICE 2',
      'CLINICAL SERVICES AGENCY NOT MISSING',
    ],
    clinical_services_agency: 'CLINICAL SERVICES AGENCY STRING',
  })
  await personLevelServices
    .updateRecordInDB('name 2', personLeveltest)
    .then((status) => {
      expect(status).not.toBeNull()
    })
  mongoose.disconnect()
...
test('add multiple records', async () => {
  ...
  const personLeveltest2 = new Object({
    uid: 'name 4',
    phase_of_service: 'PHASE OF SERVICE STRING',
    first_date: 'FIRST DATE STRING',
    ipe_with_vr: true,
  ...
  const personLeveltest3 = new Object({
    uid: 'name 5',
    phase_of_service: 'PHASE OF SERVICE STRING',
    first_date: 'FIRST DATE STRING',
    ipe_with_vr: true,
  ...
  const personLeveltest4 = new Object({
    uid: 'name 6',
    phase_of_service: 'PHASE OF SERVICE STRING',
    first_date: 'FIRST DATE STRING',
    ipe_with_vr: true,
  ...
  const personLeveltest5 = new Object({
    uid: 'name 7',
    phase_of_service: 'PHASE OF SERVICE STRING',
    first_date: 'FIRST DATE STRING',
    ipe_with_vr: true,
